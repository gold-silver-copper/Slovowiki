...
- `data/novel-words.tsv` is regenerated by every `export` from that compatible
  calibrator. The existing p≥0.6 proposal and p≥0.3 review thresholds measure
  69.6% precision / 25.0% coverage and 53.1% / 46.9% on holdout positives.
...
- **`text-check.html`** — paste Interslavic text; every token is verified
  ...
  of `check-text`);
- **`proposals.html`** ("Predloženja novyh slov") — the calibrated corpus
  proposal/review worklist with corpus-specific holdout precision and coverage;
