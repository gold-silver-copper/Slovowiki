...
/// Atomically write `bytes` to `path` (tmp file + rename), creating parent dirs.
/// The committed caches are plain JSON; the oversized enrich cache is sharded
/// (see `enrich::write_shards`) rather than compressed.
...
impl LemmaCorpus {
    pub fn load(path: &Path) -> Result<Self> {
        let bytes =
            std::fs::read(path).with_context(|| format!("open lemma corpus {}", path.display()))?;
...
pub fn extract(dump: &Path, out: &Path) -> Result<()> {
    ...
    write_atomic(out, &serde_json::to_vec(&cache)?)?;
    println!(
...
impl ProtoIndex {
    pub fn load(path: &Path) -> Result<Self> {
        let bytes =
            std::fs::read(path).with_context(|| format!("open proto cache {}", path.display()))?;
